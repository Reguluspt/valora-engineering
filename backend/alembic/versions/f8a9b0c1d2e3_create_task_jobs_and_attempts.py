...
def upgrade() -> None:
    op.create_table(
        ...
        sa.UniqueConstraint('organization_id', 'idempotency_key', name='uq_task_job_idempotency')
    )
    ...
    op.create_table(
        ...
        sa.Column('status', sa.String(length=50), nullable=False),  # running, succeeded, failed, timed_out
