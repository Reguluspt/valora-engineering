...
def upgrade() -> None:
    op.create_table(
        ...
        sa.UniqueConstraint(
            "organization_id", "bundle_code", name="uq_dossier_bundle_code"
        ),
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["dossier_bundle_id"], ["dossier_bundles.id"], ondelete="RESTRICT"
        ),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "dossier_bundle_id", "file_role", name="uq_dossier_bundle_file_role"
        ),
    )
