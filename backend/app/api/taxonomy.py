...
from app.modules.project_master_data.taxonomy_schemas import (
    ...
    AssetAttributeDefinitionUpdate,
    AssetAttributeDefinitionResponse,
)
...
def validate_node_hierarchy(db: Session, parent_id: Optional[uuid.UUID], level: TaxonomyNodeLevel):
    if level == TaxonomyNodeLevel.DOMAIN:
        if parent_id is not None:
            raise HTTPException(status_code=422, detail="Root domain node cannot have a parent")
    else:
        if parent_id is None:
            raise HTTPException(
                status_code=422, detail=f"Non-root node of level {level} requires a parent"
            )
        parent = db.query(TaxonomyNode).filter(TaxonomyNode.id == parent_id).first()
        if not parent:
            raise HTTPException(status_code=422, detail="Parent node not found")

        # Verify parent status is active
        if parent.status in [TaxonomyStatus.DEPRECATED, TaxonomyStatus.REJECTED]:
            raise HTTPException(
                status_code=422, detail="Cannot create node under deprecated or rejected parent"
            )
...
def create_family(
    ...
    if node.status != TaxonomyStatus.ACTIVE:
        raise HTTPException(
            status_code=400, detail="AssetFamily can only be created under active TaxonomyNode"
        )
...
@router.post(
    "/attribute-definitions", response_model=AssetAttributeDefinitionResponse, status_code=201
)
def create_attribute_definition(
    ...
    dup = (
        db.query(AssetAttributeDefinition)
        .filter(
    ...
    if payload.data_type == AssetAttributeDataType.ENUM and not payload.enum_values:
        raise HTTPException(status_code=422, detail="Enum attribute type requires enum_values")
...
@router.get(
    "/attribute-definitions/{attribute_id}", response_model=AssetAttributeDefinitionResponse
)
def get_attribute_definition(attribute_id: uuid.UUID, db: Session = Depends(get_db)):
    attr = (
        db.query(AssetAttributeDefinition)
        .filter(AssetAttributeDefinition.id == attribute_id)
        .first()
    )
    if not attr:
