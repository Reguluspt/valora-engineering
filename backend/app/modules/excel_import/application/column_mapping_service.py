...
def _new_profile(
    ...
    *,
    ...
    db.add(profile)
=======
    # Scalar tenant FKs do not give SQLAlchemy a relationship edge for flush
    # ordering, so persist the parent before adding its field rows.
>>>>>>> e8a6963 (fix(ci): close PostgreSQL migration and concurrency gaps)
    db.flush([profile])
    for field in fields:
        db.add(
            ColumnMappingField(
