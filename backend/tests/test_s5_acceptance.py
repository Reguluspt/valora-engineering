...
def setup_s5_rbac(db_session: Session):
    ...
    customer = Customer(organization_id=org.id, legal_name="Cust S5", status="active", created_by=user_admin.id)
...
def test_s5_comprehensive_e2e_flow(client: TestClient, setup_s5_rbac, db_session: Session) -> None:
    ...
    res = client.post(f"/api/v1/document-engine/templates/{template_id}/versions", json=v1_payload, headers=admin_headers)
    assert res.status_code == 201
    ...
    res = client.post(f"/api/v1/document-engine/template-versions/{version_id}/placeholders", json=pl_payload, headers=admin_headers)
    assert res.status_code == 201
    ...
    res = client.post(f"/api/v1/document-engine/template-versions/{version_id}/bindings", json=bind_payload, headers=admin_headers)
    assert res.status_code == 201
    ...
    res = client.post(f"/api/v1/document-engine/template-versions/{version_id}/computed-expressions", json=expr_payload, headers=admin_headers)
    assert res.status_code == 201
    ...
    docs = db_session.query(GeneratedDocument).filter(GeneratedDocument.render_job_id == uuid.UUID(job_id)).all()
    assert len(docs) == 1
    doc_id = docs[0].id
    ...
    res = client.post(f"/api/v1/document-engine/packages/{pkg_id}/items", json=item_payload, headers=admin_headers)
    assert res.status_code == 201
    ...
    res = client.post(f"/api/v1/document-intelligence/corrections/{corr_id}/review", json=review_payload, headers=admin_headers)
    ...
    tpl_audit = db_session.query(AuditEvent).filter(AuditEvent.entity_id == uuid.UUID(template_id)).first()
    assert tpl_audit is not None
    assert tpl_audit.event_name == "DOCUMENT_TEMPLATE_CREATE"

    tpl_action = db_session.query(UserActionLog).filter(UserActionLog.target_id == uuid.UUID(template_id)).first()
