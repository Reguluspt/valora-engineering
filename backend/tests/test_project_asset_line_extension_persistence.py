...
def test_project_asset_line_identity_nullable(db_session: Session) -> None:
    ...
    user = User(
        organization_id=org.id, email="pal@test.com", full_name="User", status=UserStatus.ACTIVE
    )
    db_session.add(user)
    db_session.commit()
    ...
    line = ProjectAssetLine(project_id=project.id, asset_name="Standard Asset Line", quantity=1.0)
    db_session.add(line)
    db_session.commit()
...
def test_project_asset_line_identity_relationships(db_session: Session) -> None:
    ...
    user = User(
        organization_id=org.id, email="pal2@test.com", full_name="User", status=UserStatus.ACTIVE
    )
    db_session.add(user)
    db_session.commit()
