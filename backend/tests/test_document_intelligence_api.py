...
def test_parsed_documents_and_extracted_fields(client: TestClient, setup_rbac_users) -> None:
    ...
    res = client.post(
        "/api/v1/document-intelligence/parsed-documents", json=payload, headers=admin_headers
    )
    ...
    res = client.get(
        f"/api/v1/document-intelligence/parsed-documents/{doc_id}", headers=viewer_headers
    )
    assert res.status_code == 200
    assert res.json()["confidence_score"] == 0.9500

    # 3. Patch ParsedDocument
    patch_payload = {"parse_status": "parsed", "expected_row_version": doc["row_version"]}
    res = client.patch(
        f"/api/v1/document-intelligence/parsed-documents/{doc_id}",
        json=patch_payload,
    ...
    res = client.post(
        f"/api/v1/document-intelligence/parsed-documents/{doc_id}/fields",
        json=field_payload,
        headers=admin_headers,
    )
    assert res.status_code == 201
    ...
    field_patch = {"status": "accepted", "expected_row_version": field["row_version"]}
    res = client.patch(
        f"/api/v1/document-intelligence/fields/{field['id']}",
        json=field_patch,
        headers=admin_headers,
    )
    assert res.status_code == 200
...
def test_document_correction_and_review(
    ...
    res = client.get(
        f"/api/v1/document-intelligence/corrections/{corr['id']}", headers=admin_headers
    )
    assert res.status_code == 200
    ...
    res = client.post(
        f"/api/v1/document-intelligence/corrections/{corr['id']}/review",
        json=review_payload,
        headers=admin_headers,
    )
    assert res.status_code == 200
...
def test_audit_logs_created(client: TestClient, setup_rbac_users, db_session: Session) -> None:
    ...
    audit_events = (
        db_session.query(AuditEvent).filter(AuditEvent.entity_id == uuid.UUID(doc_id)).all()
    )
    ...
    action_logs = (
        db_session.query(UserActionLog).filter(UserActionLog.target_id == uuid.UUID(doc_id)).all()
    )
