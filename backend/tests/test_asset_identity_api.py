...
def setup_user_and_headers(db_session: Session):
    ...
    fam = AssetFamily(
        taxonomy_node_id=node.id, code="FAM-T", name_vi="Family T", status=AssetFamilyStatus.ACTIVE
    )
    db_session.add(fam)
    db_session.commit()
...
def test_asset_identity_rbac_and_flow(
    ...
    audit = (
        ...
        .filter(AuditEvent.event_name == "CANONICAL_ASSET_CREATE")
        .first()
    )
    ...
    resp = client.post(
        ...
        headers=headers_admin,
        json={"alias_scope": "canonical", "raw_alias": "MBA ABB 110kV"},
    )
    assert resp.status_code == 201
    ...
    resp = client.post(
        "/api/v1/asset-identity/assets/{}/aliases".format(asset_id),
        headers=headers_admin,
        json={"alias_scope": "variant", "raw_alias": "MBA ABB 10MVA"},
    )
    ...
    resp = client.get(
        "/api/v1/asset-identity/assets/{}/variants".format(asset_id), headers=headers_unauth
    )
    assert resp.status_code == 403
    ...
    resp = client.get(
        "/api/v1/asset-identity/assets/{}/variants".format(asset_id), headers=headers_viewer
    )
    assert resp.status_code == 200
    ...
    audit_update = (
        db_session.query(AuditEvent)
        .filter(AuditEvent.event_name == "CANONICAL_ASSET_UPDATE")
        .first()
    )
    assert audit_update is not None


def test_canonical_asset_validation(
    client: TestClient, db_session: Session, setup_user_and_headers
) -> None:
    headers_admin = setup_user_and_headers["headers_admin"]
    node_id = setup_user_and_headers["taxonomy_node_id"]
...
def test_asset_variant_boundary(
    client: TestClient, db_session: Session, setup_user_and_headers
) -> None:
    headers_admin = setup_user_and_headers["headers_admin"]
    fam_id = setup_user_and_headers["asset_family_id"]
    ...
    c1 = CanonicalAsset(
        asset_family_id=fam_id,
        primary_taxonomy_node_id=node_id,
    ...
    resp = client.post(
        ...
        json={
            "asset_family_id": str(fam_id),
            "code": "VAR-SHARED-CODE",
            "display_name": "Variant for C1",
        },
    )
    assert resp.status_code == 201
    ...
    resp = client.post(
        ...
        json={
            "asset_family_id": str(fam_id),
            "code": "VAR-SHARED-CODE",
            "display_name": "Variant for C2",
        },
    )
    assert resp.status_code == 201


def test_asset_alias_scoping(
    client: TestClient, db_session: Session, setup_user_and_headers
) -> None:
    headers_admin = setup_user_and_headers["headers_admin"]
    fam_id = setup_user_and_headers["asset_family_id"]
    ...
    resp = client.post(
        ...
        headers=headers_admin,
        json={"alias_scope": "canonical", "asset_variant_id": str(v1.id), "raw_alias": "Raw 1"},
    )
    assert resp.status_code == 422
    ...
    resp = client.post(
        ...
        headers=headers_admin,
        json={"alias_scope": "variant", "asset_variant_id": str(v1.id), "raw_alias": "Raw 2"},
    )
    assert resp.status_code == 422


def test_no_hard_delete_restrictions(
    client: TestClient, db_session: Session, setup_user_and_headers
) -> None:
