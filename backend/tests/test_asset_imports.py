...
def test_excel_import_staging_flow(client: TestClient, db_session: Session) -> None:
    ...
    role_viewer = Role(code="viewer", display_name="Viewer", permissions=["project:read"])
    ...
    db_session.add_all(
        [
            ...
            UserRole(
                user_id=user_other.id, role_id=role_editor.id, is_active=True
            ),  # Editor in Org 2
        ]
    )
    db_session.commit()
    ...
    payload = {"source_filename": "assets.xlsx", "source_sheet_name": "ValuationSheet"}
    res = client.post(
        f"/api/v1/projects/{proj.id}/asset-imports", json=payload, headers=headers_editor
    )
    ...
    res_viewer = client.post(
        f"/api/v1/projects/{proj.id}/asset-imports", json=payload, headers=headers_viewer
    )
    assert res_viewer.status_code == 403

    # Test C: Create Batch multi-tenant check (user_other is in org_other, proj is in org)
    res_other = client.post(
        f"/api/v1/projects/{proj.id}/asset-imports", json=payload, headers=headers_other
    )
    assert res_other.status_code == 404

    # Test D: List Import Batches (authorized viewer)
    res_list = client.get(f"/api/v1/projects/{proj.id}/asset-imports", headers=headers_viewer)
    ...
    res_rows_empty = client.get(
        f"/api/v1/projects/{proj.id}/asset-imports/{batch_id}/rows", headers=headers_viewer
    )
    ...
    row2 = ProjectAssetImportStagingRow(
        ...
        validation_errors=[
            {"field": "proposed_quantity", "message_key": "excel.validation.invalid_number"}
        ],
    ...
    res_rows_other = client.get(
        f"/api/v1/projects/{proj.id}/asset-imports/{batch_id}/rows", headers=headers_other
    )
    assert res_rows_other.status_code == 404

    # Test I: Cross-org project listing batches is blocked
    res_list_other = client.get(f"/api/v1/projects/{proj.id}/asset-imports", headers=headers_other)
    assert res_list_other.status_code == 404

    # Test J: Batch outside project lookup is blocked
    res_bad_project = client.get(
        f"/api/v1/projects/{proj_other.id}/asset-imports/{batch_id}/rows", headers=headers_other
    )
...
def test_excel_upload_and_parser(client: TestClient, db_session: Session) -> None:
    ...
    # 1. Seed tenant, users, and roles
    org = OrganizationProfile(
        legal_name="Org A", organization_slug="org-a", status=OrganizationStatus.ACTIVE
    )
    org_other = OrganizationProfile(
        legal_name="Org B", organization_slug="org-b", status=OrganizationStatus.ACTIVE
    )
    db_session.add_all([org, org_other])
    db_session.commit()
    ...
    ws.append(
        ["Máy phát điện", "Máy phát điện diesel 5kVA", "2", "cái", "15000000", "VND", "14500000"]
    )
    # Row with formula
    ...
    staging_rows = (
        db_session.query(ProjectAssetImportStagingRow)
