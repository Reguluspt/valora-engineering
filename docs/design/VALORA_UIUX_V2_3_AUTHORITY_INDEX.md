...
5 entry point chuẩn:
- `Xem lịch sử thay đổi` — ai làm gì, khi nào, thay đổi gì.
- `Xem nguồn gốc` — dữ liệu/giá trị đến từ đâu.
- `Xem quyết định` — ai xác nhận/chọn/xử lý và vì sao.
- `Xem phiên bản` / `Xem chuỗi phiên bản` — version/snapshot lineage.
- `Xem Release Manifest` — release bind chính xác những Document Revision nào.

Context-first rule: entry point đặt gần business object cần truy vết; chỉ hiện capability có ý nghĩa. Không dùng generic `Xem Audit` trên toàn sản phẩm. `AuditEvent`, `UserActionLog`, `KnowledgeLineage`, `IdentityDecisionLog` và các persistence primitives khác không phải primary UI labels.

Deep-link phải giữ context + `return_target` để quay lại đúng asset line/document/knowledge item/quote/release. Missing/broken link phải hiển thị `Chưa được ghi nhận`, `Không khả dụng` hoặc `Không còn quyền truy cập` theo tình trạng thật; không suy diễn lineage, tự bind source thay thế hoặc che lỗi thành Empty.

Domain histories vẫn độc lập: case/price history, Knowledge lineage, Document lineage và Release history có thể deep-link qua lại nhưng không duplicate thành một timeline toàn cục. Visual board là pattern authority cho decision tree, cross-product entry-point map, context contract, lineage mental models, missing-link behavior, UX/access rules và Do/Don't.

Nếu visual wording mâu thuẫn semantic contract/domain authority, semantic/domain authority thắng.

## 3. Microsoft 365 Return / Revalidation authority
...
## 5. Post-Publish Success authority
...
## 6. Tổng quan hồ sơ / orchestration authority
...
## 8. Knowledge Management authority
